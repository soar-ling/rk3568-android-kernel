...
*   WKIC Ltd.
*   By  Xu XunWei Tech  
*   DEMO Version :2.4.2 Data:2022-06-27
...
#define VERSION_DESC  "V2.4.2 On 2022.06.27"
/*************The debug control **********************************/
...
#define 	WK_CRASTAL_CLK		        (12000000)
...
struct wk2xxx_port 
{
    ...
    struct gpio_desc *rs485_enable_gpio;
};
...
{   
    ...
    #ifdef _DEBUG_WK_RX
     printk(KERN_ALERT "rx_chars()-port:%lx--fsr:0x%x--rfcnt:%d--\n",one->port.iobase,fsr,rfcnt);
    #endif
    flg = TTY_NORMAL;
        ...
        #else
	        for(rx_num=0;rx_num<relen;rx_num++){
		        wk2xxx_read_slave_reg(s->spi_wk,one->port.iobase,WK2XXX_FDAT,dat);
		        rx_dat[rx_num]=dat[0];
		    }
        #endif
...
static void wk2xxx_stop_tx(struct uart_port *port)//
{
    ...
    mutex_lock(&wk2xxxs_lock);
    if(port->iobase == WK2XXX_485_PORT){
        gpiod_direction_output(s->rs485_enable_gpio, 1);
    }
...
static void wk2xxx_start_tx(struct uart_port *port)
{
    ...
    if (one->port.iobase == WK2XXX_485_PORT) {
        gpiod_direction_output(s->rs485_enable_gpio, 0);
    }
...
{   
	...
	if(fwcr>0){  
            printk(KERN_ALERT "Flow Control  fwcr=0x%X\n", fwcr);
	...
	#ifdef _DEBUG_WK_FUNCTION
        wk2xxx_read_slave_reg(s->spi_wk,one->port.iobase,WK2XXX_BAUD0_REG,dat);
        printk(KERN_ALERT ":WK2XXX_BAUD0=0x%X\n", dat[0]);
        wk2xxx_read_slave_reg(s->spi_wk,one->port.iobase,WK2XXX_BAUD1_REG,dat);
        printk(KERN_ALERT ":WK2XXX_BAUD1=0x%X\n", dat[0]);
        wk2xxx_read_slave_reg(s->spi_wk,one->port.iobase,WK2XXX_PRES_REG,dat);
...
static int wk2xxx_spi_rstgpio_parse_dt(struct device *dev,int *rst_gpio)
{
	...
	int rst_flags; 
        ...
        printk(KERN_ALERT "%s!!--in--\n", __func__);
	#endif
	*rst_gpio = of_get_named_gpio_flags(dev->of_node, "reset_gpio", 0,(enum of_gpio_flags *)&rst_flags);
...
static int wk2xxx_probe(struct spi_device *spi)
{
    ...
    //struct sched_param sched_param = { .sched_priority = 4 / 2 };
    uint8_t i;
    int status,ret, irq;
    uint8_t dat[1];
    ...
    wk2xxx_spi_rstgpio_parse_dt(&spi->dev,&s->rst_gpio_num);
	/*reset wk2xxx*/
    mdelay(10);
    ...
    s->rs485_enable_gpio = devm_gpiod_get_optional(&spi->dev, "rs485_en", GPIOD_ASIS);
    if (IS_ERR(s->rs485_enable_gpio)) {
    ...
    for(i =0;i<NR_PORTS;i++){
		...
		s->p[i].port.flags    = (1U << 28);//ASYNC_BOOT_AUTOCONF;
