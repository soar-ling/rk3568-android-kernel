...
int rk628_pinctrl_set_mux(struct rk628 *rk628, int gpio, int mux)
{
	...
	if ((i == ARRAY_SIZE(rk628_pin_iomux_groups)) || (!iomux_base)) {
		pr_info("rk628_pinctrl_set_mux invalid gpio or iomux_base\n");
		return -1;
	}
...
int rk628_gpio_get_value(struct rk628 *rk628, int gpio)
{
	...
	if ((i == ARRAY_SIZE(rk628_pin_iomux_groups)) || (!data_reg)) {
		pr_info("rk628_gpio_get_value invalid gpio or data_reg\n");
		return -1;
	}
...
int rk628_gpio_set_value(struct rk628 *rk628, int gpio, int value)
{
	...
	if ((i == ARRAY_SIZE(rk628_pin_iomux_groups)) || (!data_reg)) {
		pr_info("rk628_gpio_set_value invalid gpio or data_reg\n");
		return -1;
	}
...
int rk628_gpio_set_direction(struct rk628 *rk628, int gpio, int direction)
{
	...
	if ((i == ARRAY_SIZE(rk628_pin_iomux_groups)) || (!dir_reg)) {
		pr_info("rk628_gpio_set_direction invalid gpio or dir_reg\n");
		return -1;
	}
...
{
	rk628_pinctrl_set_mux(rk628, gpio, GPIO_FUNC);

	rk628_gpio_set_direction(rk628, gpio, GPIO_DIRECTION_IN);
...
int rk628_gpio_direction_output(struct rk628 *rk628, int gpio, int value)
{
	...
	rk628_pinctrl_set_mux(rk628, gpio, GPIO_FUNC);
	rk628_gpio_set_value(rk628, gpio, value);
	rk628_gpio_set_direction(rk628, gpio, GPIO_DIRECTION_OUT);
	return 0;
}
...
int rk628_gpio_test_all(struct rk628 *rk628)
{
	...
	for (i = 0; i < ARRAY_SIZE(rk628_pin_iomux_groups); i++) {
		...
		rk628_gpio_direction_output(rk628, rk628_pin_iomux_groups[i].pins, 1);
	}
	...
	for (i = 0; i < ARRAY_SIZE(rk628_pin_iomux_groups); i++) {
		if (rk628_pin_iomux_groups[i].pins && (rk628_pin_iomux_groups[i].pins != GPIO1_A1)
			...
			&& (rk628_pin_iomux_groups[i].pins != GPIO2_C3)
			&& (rk628_pin_iomux_groups[i].pins != GPIO2_C4))
		rk628_gpio_direction_output(rk628, rk628_pin_iomux_groups[i].pins, 0);
	}
