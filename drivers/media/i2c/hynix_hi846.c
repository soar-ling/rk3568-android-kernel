...
static void hi846_get_otp(struct otp_info *otp,
...
static int hi846_s_stream(struct v4l2_subdev *sd, int on)
{
	...
	dev_info(&client->dev, "zc:%s: on: %d, %dx%d@%d\n", __func__, on,
	...
	if (on) {
		dev_info(&client->dev, "zc: stream on!!!\n");
