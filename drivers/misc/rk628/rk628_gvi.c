...
static void rk628_gvi_get_info(struct rk628_gvi *gvi)
{
	...
	default:
		gvi->byte_mode = 3;
		gvi->color_depth = COLOR_DEPTH_RGB_YUV444_24BIT;
		printk("unsupported bus_format: 0x%x\n", gvi->bus_format);
		break;
	}
}
...
{
	int i;
	//const int rk628f_lanes[RK628_GVI_LANES_MAX] = {2, 3, 0, 1, 6, 7, 4, 5};
				       ...
				       struct rk628_gvi *gvi)
{
	...
	u16 vm_hactive, vm_hfront_porch, vm_hback_porch, vm_hsync_len;
	u16 vm_vactive, vm_vfront_porch, vm_vback_porch, vm_vsync_len;
	u16 hsync_len, hact_st, hact_end, htotal;
	u16 vsync_len, vact_st, vact_end, vtotal;

	vm_hactive = mode->hdisplay;
	vm_hfront_porch = mode->hsync_start - mode->hdisplay;
	vm_hsync_len = mode->hsync_end - mode->hsync_start;
	vm_hback_porch = mode->htotal - mode->hsync_end;

	vm_vactive = mode->vdisplay;
	vm_vfront_porch = mode->vsync_start - mode->vdisplay;
