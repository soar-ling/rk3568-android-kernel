...
static int rk628_debugfs_dump(struct seq_file *s, void *data)
{
	...
	if(input_mode == 0){
		...
		DEBUG_PRINT("\tH: %d %d %d %d\n",src_hactive,src_htotal - src_hoffset,src_htotal - src_hoffset + src_hs_end, src_htotal);

		DEBUG_PRINT("\tV: %d %d %d %d\n",src_vactive,src_vtotal - src_voffset - src_vs_end,src_vtotal - src_voffset, src_vtotal);
	...
	DEBUG_PRINT("\tH: %d %d %d %d\n",dsp_hact_end - dsp_hact_st,dsp_htotal - dsp_hact_st,dsp_htotal - dsp_hact_st + dsp_hs_end, dsp_htotal);
	DEBUG_PRINT("\tV: %d %d %d %d\n",dsp_vact_end - dsp_vact_st,dsp_vtotal - dsp_vact_st, dsp_vtotal - dsp_vact_st + dsp_vs_end, dsp_vtotal);
...
rk628_i2c_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	if (rk628_output_is_hdmi(rk628)) {
		if (of_property_read_bool(dev->of_node, "hdmitx-cec-enable")) {
			rk628->hdmitx_cec_enable = true;
			dev_info(dev, "hdmitx cec enable\n");
		}
	}
