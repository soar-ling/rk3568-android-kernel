 ...
 * Shunqing Chen <csq@rock-chisp.com>
 *
...
static u8 testif_write(struct rk628 *rk628, u8 test_code, u8 test_data)
{
	...
	printk("test_code=0x%02x, ", test_code);
	printk("test_data=0x%02x, ", test_data);
	printk("monitor_data=0x%02x\n", monitor_data);
...
static int mipi_dphy_reset(struct rk628 *rk628)
{
	...
	if (ret < 0) {
		printk("lane module is not in stop state\n");
		return ret;
	}
...
static int mipi_dphy_power_on(struct rk628 *rk628)
{
	...
	printk("%s mipi bitrate:%llu mbps\n", __func__, csi.lane_mbps);
...
static void rk628_csi_set_csi(struct rk628 *rk628)
{
	...
	if (csi.txphy_pwron) {
		printk("%s: txphy already power on, power off\n",
			__func__);
...
static void enable_csitx(struct rk628 *rk628)
{
	...
	for (i = 0; i < CSITX_ERR_RETRY_TIMES; i++) {
		...
		printk("%s csitx err, retry:%d, err status:%#x, ret:%d\n",
		       __func__, i, val, ret);
	}
...
{
	rk628_i2c_update_bits(rk628, GRF_SYSTEM_CON0, SW_OUTPUT_MODE_MASK, SW_OUTPUT_MODE(OUTPUT_MODE_CSI));
