...
struct rockchip_canfd {
	...
	u32 noack_cnt;
	u32 noack_timeout_cnt;
	u32 noack_stop_req;
	u32 noack_stop_check;
	u32 delay_time;
};
...
static void rockchip_canfd_tx_err_delay_work(struct work_struct *work)
{
	...
	if ((err_code & NOACK_ERR_FLAG) == NOACK_ERR_FLAG) {
		rcan->noack_cnt++;
		if (rcan->noack_cnt == rcan->noack_timeout_cnt) {
			rcan->noack_stop_req = 1;
			rcan->noack_stop_check = 1;
		} else if (rcan->noack_cnt > rcan->noack_timeout_cnt) {
			rcan->noack_stop_req = 1;
		}
		rockchip_canfd_write(rcan, CAN_CMD, CAN_TX0_REQ);
		schedule_delayed_work(&rcan->tx_err_work, rcan->delay_time);
		return;		
	} else {
		rockchip_canfd_write(rcan, CAN_MODE, 0);
...
static irqreturn_t rockchip_canfd_interrupt(int irq, void *dev_id)
{
	...
	if (isr & TX_FINISH_INT) {
		cancel_delayed_work(&rcan->tx_err_work);
		if (rcan->noack_stop_check) {
			rcan->noack_stop_check = 0;
			rcan->noack_stop_req = 0;
			rcan->noack_cnt = 0;
		}
