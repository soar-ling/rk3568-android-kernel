...
#include "halmac_usb_8822e.h"
#endif
#include "halmac_gpio_8822e.h"
#include "halmac_common_8822e.h"
#include "halmac_cfg_wmac_8822e.h"
...
#define RSVD_PG_DLLB_NUM		(TX_FIFO_SIZE_8822E / 3 >> \
...
static struct halmac_rqpn HALMAC_RQPN_SDIO_8822E[] = {
...
/*PCIE RQPN Mapping*/
static struct halmac_rqpn HALMAC_RQPN_PCIE_8822E[] = {
	/* { mode, vo_map, vi_map, be_map, bk_map, mg_map, hi_map } */
	{HALMAC_TRX_MODE_NORMAL,
	 HALMAC_MAP2_NQ, HALMAC_MAP2_NQ, HALMAC_MAP2_LQ, HALMAC_MAP2_LQ,
	 HALMAC_MAP2_EXQ, HALMAC_MAP2_HQ},
...
static struct halmac_rqpn HALMAC_RQPN_2BULKOUT_8822E[] = {
	/* { mode, vo_map, vi_map, be_map, bk_map, mg_map, hi_map } */
	{HALMAC_TRX_MODE_NORMAL,
...
static struct halmac_rqpn HALMAC_RQPN_3BULKOUT_8822E[] = {
	/* { mode, vo_map, vi_map, be_map, bk_map, mg_map, hi_map } */
	{HALMAC_TRX_MODE_NORMAL,
	 HALMAC_MAP2_NQ, HALMAC_MAP2_NQ, HALMAC_MAP2_LQ, HALMAC_MAP2_LQ,
	 HALMAC_MAP2_HQ, HALMAC_MAP2_HQ},
...
static struct halmac_rqpn HALMAC_RQPN_4BULKOUT_8822E[] = {
	/* { mode, vo_map, vi_map, be_map, bk_map, mg_map, hi_map } */
	{HALMAC_TRX_MODE_NORMAL,
	 HALMAC_MAP2_NQ, HALMAC_MAP2_NQ, HALMAC_MAP2_LQ, HALMAC_MAP2_LQ,
	 HALMAC_MAP2_EXQ, HALMAC_MAP2_HQ},
...
#if HALMAC_PLATFORM_WINDOWS
/*SDIO Page Number*/
static struct halmac_pg_num HALMAC_PG_NUM_SDIO_8822E[] = {
...
/*PCIE Page Number*/
static struct halmac_pg_num HALMAC_PG_NUM_PCIE_8822E[] = {
	/* { mode, hq_num, nq_num, lq_num, exq_num, gap_num} */
	{HALMAC_TRX_MODE_NORMAL, 64, 64, 64, 64, 1},
...
static struct halmac_pg_num HALMAC_PG_NUM_2BULKOUT_8822E[] = {
	/* { mode, hq_num, nq_num, lq_num, exq_num, gap_num} */
...
static struct halmac_pg_num HALMAC_PG_NUM_3BULKOUT_8822E[] = {
	/* { mode, hq_num, nq_num, lq_num, exq_num, gap_num} */
	{HALMAC_TRX_MODE_NORMAL, 64, 64, 64, 0, 1},
...
static struct halmac_pg_num HALMAC_PG_NUM_4BULKOUT_8822E[] = {
	/* { mode, hq_num, nq_num, lq_num, exq_num, gap_num} */
	{HALMAC_TRX_MODE_NORMAL, 64, 64, 64, 64, 1},
	{HALMAC_TRX_MODE_TRXSHARE, 64, 64, 64, 64, 1},
	{HALMAC_TRX_MODE_WMM, 64, 64, 64, 64, 1},
...
txdma_queue_mapping_8822e(struct halmac_adapter *adapter,
			  enum halmac_trx_mode mode);

static enum halmac_ret_status
priority_queue_cfg_8822e(struct halmac_adapter *adapter,
			 enum halmac_trx_mode mode);

static enum halmac_ret_status
set_trx_fifo_info_8822e(struct halmac_adapter *adapter,
			enum halmac_trx_mode mode);

static void
init_txq_ctrl_8822e(struct halmac_adapter *adapter);

static void
init_sifs_ctrl_8822e(struct halmac_adapter *adapter);

static void
init_rate_fallback_ctrl_8822e(struct halmac_adapter *adapter);

enum halmac_ret_status
mount_api_8822e(struct halmac_adapter *adapter)
{
	struct halmac_api *api = (struct halmac_api *)adapter->halmac_api;

	adapter->chip_id = HALMAC_CHIP_ID_8822E;
	adapter->hw_cfg_info.efuse_size = EFUSE_SIZE_8822E;
	adapter->hw_cfg_info.eeprom_size = EEPROM_SIZE_8822E;
	adapter->hw_cfg_info.bt_efuse_size = BT_EFUSE_SIZE_8822E;
	adapter->hw_cfg_info.prtct_efuse_size = PRTCT_EFUSE_SIZE_8822E;
	adapter->hw_cfg_info.cam_entry_num = SEC_CAM_NUM_8822E;
	adapter->hw_cfg_info.tx_fifo_size = TX_FIFO_SIZE_8822E;
	adapter->hw_cfg_info.rx_fifo_size = RX_FIFO_SIZE_8822E;
	adapter->hw_cfg_info.ac_oqt_size = OQT_ENTRY_AC_8822E;
	adapter->hw_cfg_info.non_ac_oqt_size = OQT_ENTRY_NOAC_8822E;
	adapter->hw_cfg_info.usb_txagg_num = BLK_DESC_NUM;
	adapter->txff_alloc.rsvd_drv_pg_num = RSVD_PG_DRV_NUM;

	api->halmac_init_trx_cfg = init_trx_cfg_8822e;
	api->halmac_init_system_cfg = init_system_cfg_8822e;
	api->halmac_init_protocol_cfg = init_protocol_cfg_8822e;
	api->halmac_init_h2c = init_h2c_8822e;
	api->halmac_pinmux_get_func = pinmux_get_func_8822e;
	api->halmac_pinmux_set_func = pinmux_set_func_8822e;
	api->halmac_pinmux_free_func = pinmux_free_func_8822e;
	api->halmac_get_hw_value = get_hw_value_8822e;
	api->halmac_set_hw_value = set_hw_value_8822e;
	api->halmac_cfg_drv_info = cfg_drv_info_8822e;
	api->halmac_fill_txdesc_checksum = fill_txdesc_check_sum_8822e;
	api->halmac_init_low_pwr = init_low_pwr_8822e;
	api->halmac_pre_init_system_cfg = pre_init_system_cfg_8822e;

	api->halmac_init_wmac_cfg = init_wmac_cfg_8822e;
	api->halmac_init_edca_cfg = init_edca_cfg_8822e;
	
	api->halmac_dump_efuse_map = dump_efuse_map_8822e;
	api->halmac_dump_efuse_map_bt = dump_efuse_map_bt_8822e;
	api->halmac_write_efuse_bt = write_efuse_bt_8822e;
	api->halmac_read_efuse_bt = read_efuse_bt_8822e;
	api->halmac_dump_logical_efuse_map = dump_log_efuse_map_8822e;
	api->halmac_dump_logical_efuse_mask = dump_log_efuse_mask_8822e;
	api->halmac_pg_efuse_by_map = pg_efuse_by_map_8822e;
	api->halmac_mask_logical_efuse = mask_log_efuse_8822e;
	api->halmac_get_efuse_available_size = get_efuse_available_size_8822e;

	api->halmac_write_logical_efuse = write_log_efuse_8822e;
	api->halmac_write_logical_efuse_word = write_log_efuse_word_8822e;
	api->halmac_read_logical_efuse = read_logical_efuse_8822e;

	api->halmac_write_wifi_phy_efuse = write_wifi_phy_efuse_8822e;
	api->halmac_read_wifi_phy_efuse = read_wifi_phy_efuse_8822e;

	if (adapter->intf == HALMAC_INTERFACE_SDIO) {
#if HALMAC_SDIO_SUPPORT
		adapter->sdio_hw_info.tx_addr_format = HALMAC_SDIO_AGG_MODE;
		api->halmac_init_interface_cfg = init_sdio_cfg_8822e;
		api->halmac_init_sdio_cfg = init_sdio_cfg_8822e;
		api->halmac_mac_power_switch = mac_pwr_switch_sdio_8822e;
		api->halmac_phy_cfg = phy_cfg_sdio_8822e;
		api->halmac_pcie_switch = pcie_switch_sdio_8822e;
		api->halmac_interface_integration_tuning = intf_tun_sdio_8822e;
		api->halmac_tx_allowed_sdio = tx_allowed_sdio_8822e;
		api->halmac_get_sdio_tx_addr = get_sdio_tx_addr_8822e;
		api->halmac_reg_read_8 = reg_r8_sdio_8822e;
		api->halmac_reg_write_8 = reg_w8_sdio_8822e;
		api->halmac_reg_read_16 = reg_r16_sdio_8822e;
		api->halmac_reg_write_16 = reg_w16_sdio_8822e;
		api->halmac_reg_read_32 = reg_r32_sdio_8822e;
		api->halmac_reg_write_32 = reg_w32_sdio_8822e;

		adapter->sdio_fs.macid_map_size = MACID_MAX_8822E << 1;
...
#if HALMAC_USB_SUPPORT
		api->halmac_mac_power_switch = mac_pwr_switch_usb_8822e;
		api->halmac_phy_cfg = phy_cfg_usb_8822e;
		api->halmac_pcie_switch = pcie_switch_usb_8822e;
		api->halmac_interface_integration_tuning = intf_tun_usb_8822e;
#endif
	} else if (adapter->intf == HALMAC_INTERFACE_PCIE) {
#if HALMAC_PCIE_SUPPORT
		api->halmac_mac_power_switch = mac_pwr_switch_pcie_8822e;
		api->halmac_phy_cfg = phy_cfg_pcie_8822e;
		api->halmac_pcie_switch = pcie_switch_8822e;
		api->halmac_interface_integration_tuning = intf_tun_pcie_8822e;
		api->halmac_cfgspc_set_pcie = cfgspc_set_pcie_8822e;
#endif
	} else {
...
/**
 * init_trx_cfg_8822e() - config trx dma register
 ...
 */
enum halmac_ret_status
init_trx_cfg_8822e(struct halmac_adapter *adapter, enum halmac_trx_mode mode)
{
	u8 value8;
	struct halmac_api *api = (struct halmac_api *)adapter->halmac_api;
	...
	status = txdma_queue_mapping_8822e(adapter, mode);
	...
	status = priority_queue_cfg_8822e(adapter, mode);
	if (status != HALMAC_RET_SUCCESS) {
		PLTFM_MSG_ERR("[ERR]priority queue cfg\n");
		return status;
	}

	status = init_h2c_8822e(adapter);
	if (status != HALMAC_RET_SUCCESS) {
			  ...
			  enum halmac_trx_mode mode)
{
	...
	if (adapter->intf == HALMAC_INTERFACE_SDIO) {
		cur_rqpn_sel = HALMAC_RQPN_SDIO_8822E;
	} else if (adapter->intf == HALMAC_INTERFACE_PCIE) {
		cur_rqpn_sel = HALMAC_RQPN_PCIE_8822E;
	} else if (adapter->intf == HALMAC_INTERFACE_USB) {
		if (adapter->bulkout_num == 2) {
			cur_rqpn_sel = HALMAC_RQPN_2BULKOUT_8822E;
		} else if (adapter->bulkout_num == 3) {
			cur_rqpn_sel = HALMAC_RQPN_3BULKOUT_8822E;
		} else if (adapter->bulkout_num == 4) {
			cur_rqpn_sel = HALMAC_RQPN_4BULKOUT_8822E;
		} else {
			 ...
			 enum halmac_trx_mode mode)
{
	...
	status = set_trx_fifo_info_8822e(adapter, mode);
	if (status != HALMAC_RET_SUCCESS) {
	...
	if (adapter->intf == HALMAC_INTERFACE_SDIO) {
		cur_pg_num = HALMAC_PG_NUM_SDIO_8822E;
	} else if (adapter->intf == HALMAC_INTERFACE_PCIE) {
		cur_pg_num = HALMAC_PG_NUM_PCIE_8822E;
	} else if (adapter->intf == HALMAC_INTERFACE_USB) {
		if (adapter->bulkout_num == 2) {
			cur_pg_num = HALMAC_PG_NUM_2BULKOUT_8822E;
		} else if (adapter->bulkout_num == 3) {
			cur_pg_num = HALMAC_PG_NUM_3BULKOUT_8822E;
		} else if (adapter->bulkout_num == 4) {
			cur_pg_num = HALMAC_PG_NUM_4BULKOUT_8822E;
		} else {
			PLTFM_MSG_ERR("[ERR]interface not support\n");
			return HALMAC_RET_NOT_SUPPORT;
		}
...
{
	u16 cur_pg_addr;
	u32 txff_size = TX_FIFO_SIZE_8822E;
	u32 rxff_size = RX_FIFO_SIZE_8822E;
	struct halmac_txff_allocation *info = &adapter->txff_alloc;

	if (info->rx_fifo_exp_mode == HALMAC_RX_FIFO_EXPANDING_MODE_1_BLOCK) {
		txff_size = TX_FIFO_SIZE_RX_EXPAND_1BLK_8822E;
		rxff_size = RX_FIFO_SIZE_RX_EXPAND_1BLK_8822E;
	} else if (info->rx_fifo_exp_mode ==
		   HALMAC_RX_FIFO_EXPANDING_MODE_2_BLOCK) {
		txff_size = TX_FIFO_SIZE_RX_EXPAND_2BLK_8822E;
		rxff_size = RX_FIFO_SIZE_RX_EXPAND_2BLK_8822E;
	} else if (info->rx_fifo_exp_mode ==
		   HALMAC_RX_FIFO_EXPANDING_MODE_3_BLOCK) {
		txff_size = TX_FIFO_SIZE_RX_EXPAND_3BLK_8822E;
		rxff_size = RX_FIFO_SIZE_RX_EXPAND_3BLK_8822E;
	} else if (info->rx_fifo_exp_mode ==
		   HALMAC_RX_FIFO_EXPANDING_MODE_4_BLOCK) {
		txff_size = TX_FIFO_SIZE_RX_EXPAND_4BLK_8822E;
		rxff_size = RX_FIFO_SIZE_RX_EXPAND_4BLK_8822E;
	}

	if (info->la_mode != HALMAC_LA_MODE_DISABLE) {
		txff_size = TX_FIFO_SIZE_LA_8822E;
		rxff_size = RX_FIFO_SIZE_8822E;
	}
...
/**
 * init_system_cfg_8822e() -  init system config
 * @adapter : the adapter of halmac
 * Author : KaiYuan Chang/Ivan Lin
 * Return : enum halmac_ret_status
 * More details of status code can be found in prototype document
 */
enum halmac_ret_status
init_system_cfg_8822e(struct halmac_adapter *adapter)
{
	struct halmac_api *api = (struct halmac_api *)adapter->halmac_api;
	u8 value8;
	u32 tmp = 0;
	...
	if (adapter->intf == HALMAC_INTERFACE_PCIE) {
		hwval = 1;
		status = set_hw_value_8822e(adapter, HALMAC_HW_PCIE_REF_AUTOK,
					    &hwval);
		if (status != HALMAC_RET_SUCCESS)
			return status;
	}
	...
	if (adapter->intf == HALMAC_INTERFACE_SDIO) {
		...
		status = set_hw_value_8822e(adapter, HALMAC_HW_SDIO_TX_FORMAT,
					    &sdio_tx_format);
		if (status != HALMAC_RET_SUCCESS)
			return status;
	}
...
/**
 * init_protocol_cfg_8822e() - config protocol register
 * @adapter : the adapter of halmac
 * Author : KaiYuan Chang/Ivan Lin
 * Return : enum halmac_ret_status
 * More details of status code can be found in prototype document
 */
enum halmac_ret_status
init_protocol_cfg_8822e(struct halmac_adapter *adapter)
{
	u32 max_agg_num;
	u32 max_rts_agg_num;
	u32 value32;
	...
	init_txq_ctrl_8822e(adapter);
	init_sifs_ctrl_8822e(adapter);
	init_rate_fallback_ctrl_8822e(adapter);
	...
	value32 = BIT_CLEAR_RRSR_RSC_8822E(HALMAC_REG_R32(REG_RRSR));
...
/**
 * init_h2c_8822e() - config h2c packet buffer
 * @adapter : the adapter of halmac
 * Author : KaiYuan Chang/Ivan Lin
 * Return : enum halmac_ret_status
 * More details of status code can be found in prototype document
 */
enum halmac_ret_status
init_h2c_8822e(struct halmac_adapter *adapter)
{
	u8 value8;
	u32 value32;
	u32 h2cq_addr;
...
/**
 * init_edca_cfg_8822e() - init EDCA config
 * @adapter : the adapter of halmac
 * Author : KaiYuan Chang/Ivan Lin
 * Return : enum halmac_ret_status
 * More details of status code can be found in prototype document
 */
enum halmac_ret_status
init_edca_cfg_8822e(struct halmac_adapter *adapter)
{
	u8 value8;
	struct halmac_api *api = (struct halmac_api *)adapter->halmac_api;
...
/**
 * init_wmac_cfg_8822e() - init wmac config
 * @adapter : the adapter of halmac
 * Author : KaiYuan Chang/Ivan Lin
 * Return : enum halmac_ret_status
 * More details of status code can be found in prototype document
 */
enum halmac_ret_status
init_wmac_cfg_8822e(struct halmac_adapter *adapter)
{
	u8 value8;
	struct halmac_api *api = (struct halmac_api *)adapter->halmac_api;
	enum halmac_ret_status status = HALMAC_RET_SUCCESS;
...
/**
 * pre_init_system_cfg_8822e() - pre-init system config
 * @adapter : the adapter of halmac
 * Author : KaiYuan Chang/Ivan Lin
 * Return : enum halmac_ret_status
 * More details of status code can be found in prototype document
 */
enum halmac_ret_status
pre_init_system_cfg_8822e(struct halmac_adapter *adapter)
{
	u32 value32;
	struct halmac_api *api = (struct halmac_api *)adapter->halmac_api;
	u8 enable_bb;
