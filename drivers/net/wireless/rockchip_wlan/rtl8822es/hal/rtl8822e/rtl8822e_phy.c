 ...
 *	_FALSE	Fail
 */
u8 rtl8822e_phy_init_mac_register(PADAPTER adapter)
{
	PHAL_DATA_TYPE hal;
	u8 ret = _TRUE;
	int res;
 ...
 *	_FALSE	Fail
 */
u8 rtl8822e_phy_init(PADAPTER adapter)
{
	struct dvobj_priv *d;
	...
	ret = config_phydm_parameter_init_8822e(phydm, ODM_PRE_SETTING);
	if (FALSE == ret)
		return _FALSE;
	...
	ret = config_phydm_parameter_init_8822e(phydm, ODM_POST_SETTING);
	if (FALSE == ret)
		return _FALSE;
...
static void init_phydm_cominfo(PADAPTER adapter)
{
	...
	odm_cmn_info_init(p_dm_odm, ODM_CMNINFO_IC_TYPE, ODM_RTL8822E);
...
void rtl8822e_phy_deinit_dm_priv(PADAPTER adapter)
{
	struct dm_struct *podmpriv = adapter_to_phydm(adapter);
...
void rtl8822e_phy_init_haldm(PADAPTER adapter)
{
...
static void check_rxfifo_full(PADAPTER adapter)
{
	...
	if (regsty->check_hw_status == 1) {
		...
		val8 = rtw_read8(adapter, REG_RXERR_RPT_8822E + 3);
		rtw_write8(adapter, REG_RXERR_RPT_8822E + 3, (val8 | 0xa0));

		pdbgpriv->dbg_rx_fifo_last_overflow = pdbgpriv->dbg_rx_fifo_curr_overflow;
		pdbgpriv->dbg_rx_fifo_curr_overflow = rtw_read16(adapter, REG_RXERR_RPT_8822E);
		pdbgpriv->dbg_rx_fifo_diff_overflow =
			pdbgpriv->dbg_rx_fifo_curr_overflow - pdbgpriv->dbg_rx_fifo_last_overflow;
	}
}

void rtl8822e_phy_haldm_watchdog(PADAPTER adapter)
{
...
u32 rtl8822e_read_bb_reg(PADAPTER adapter, u32 addr, u32 mask)
{
...
void rtl8822e_write_bb_reg(PADAPTER adapter, u32 addr, u32 mask, u32 val)
{
	u32 val_org, shift;
...
u32 rtl8822e_read_rf_reg(PADAPTER adapter, enum rf_path path, u32 addr, u32 mask)
{
	struct dm_struct *phydm = adapter_to_phydm(adapter);
	u32 val;

	val = config_phydm_read_rf_reg_8822e(phydm, path, addr, mask);
	if (!config_phydm_read_rf_check_8822e(val))
...
void rtl8822e_write_rf_reg(PADAPTER adapter, enum rf_path path, u32 addr, u32 mask, u32 val)
{
	struct dm_struct *phydm = adapter_to_phydm(adapter);
	u8 ret;

	ret = config_phydm_write_rf_reg_8822e(phydm, path, addr, mask, val);
	if (_FALSE == ret)
...
void rtl8822e_set_tx_power_level(PADAPTER adapter, u8 channel)
{
	struct hal_spec_t *hal_spec = GET_HAL_SPEC(adapter);
	u8 path;
...
void rtl8822e_set_txpwr_done(_adapter *adapter)
{
	HAL_DATA_TYPE *hal_data = GET_HAL_DATA(adapter);
	struct dm_struct *phydm = adapter_to_phydm(adapter);

	config_phydm_set_txagc_to_hw_8822e(phydm);
...
u8 rtl8822e_get_dis_dpd_by_rate_diff(PADAPTER adapter, u8 rate)
{
	struct dm_struct *phydm = adapter_to_phydm(adapter);
	u16 dis_dpd_rate;
	u8 dis_dpd_rate_diff = 0;

	dis_dpd_rate = phydm_get_dis_dpd_by_rate_8822e(phydm);
	switch (rate) {
		case MGN_6M:
 ...
 */
void rtl8822e_set_tx_power_index(PADAPTER adapter, u32 powerindex, enum rf_path rfpath, u8 rate)
{
	...
	*/
	...
	rate = rate & 0xFC;
	write_ret = config_phydm_write_txagc_8822e(phydm, hal->txagc_set_buf, rfpath, rate);
...
static void switch_chnl_and_set_bw_by_drv(PADAPTER adapter, u8 switch_band)
{
	...
	if (switch_band) {
		...
		ret = config_phydm_switch_band_8822e(p_dm_odm, hal->current_channel);

		if (!ret) {
			RTW_INFO("%s: config_phydm_switch_band_8822e fail\n", __FUNCTION__);
			rtw_warn_on(1);
			return;
		}
	}
	...
	if (hal->bSwChnl) {
		ret = config_phydm_switch_channel_8822e(p_dm_odm, hal->current_channel);
		hal->bSwChnl = _FALSE;

		if (!ret) {
			RTW_INFO("%s: config_phydm_switch_channel_8822e fail\n", __FUNCTION__);
			rtw_warn_on(1);
			return;
		}
	}
...
#ifdef CONFIG_NARROWBAND_SUPPORTING
		if (adapter->registrypriv.rtw_nb_config == RTW_NB_CONFIG_WIDTH_10) {
			rtw_write8(adapter, REG_CCK_CHECK_8822E,
				(rtw_read8(adapter, REG_CCK_CHECK_8822E) | BIT_CHECK_CCK_EN_8822E));
			ret = config_phydm_switch_bandwidth_8822e(p_dm_odm, pri_ch_idx, CHANNEL_WIDTH_10);
		} else if (adapter->registrypriv.rtw_nb_config == RTW_NB_CONFIG_WIDTH_5) {
			rtw_write8(adapter, REG_CCK_CHECK_8822E,
				(rtw_read8(adapter, REG_CCK_CHECK_8822E) | BIT_CHECK_CCK_EN_8822E));
			ret = config_phydm_switch_bandwidth_8822e(p_dm_odm, pri_ch_idx, CHANNEL_WIDTH_5);
		} else
#endif
		ret = config_phydm_switch_bandwidth_8822e(p_dm_odm, pri_ch_idx, hal->current_channel_bw);
		hal->bSetChnlBW = _FALSE;

		if (!ret) {
			RTW_INFO("%s: config_phydm_switch_bandwidth_8822e fail\n", __FUNCTION__);
			rtw_warn_on(1);
			return;
		}
	}
...
void rtl8822e_handle_sw_chnl_and_set_bw(
	...
	u8 ChnlOffsetOf80MHz, u8 CenterFrequencyIndex1)
{
	...
	if (!RTW_CANNOT_RUN(Adapter))
		rtl8822e_switch_chnl_and_set_bw(Adapter);
	else {
		if (hal->bSwChnl) {
 ...
 */
void rtl8822e_set_channel_bw(PADAPTER adapter, u8 center_ch, enum channel_width bw, u8 offset40, u8 offset80)
{
	rtl8822e_handle_sw_chnl_and_set_bw(adapter, _TRUE, _TRUE, center_ch, bw, offset40, offset80, center_ch);
}

void rtl8822e_notch_filter_switch(PADAPTER adapter, bool enable)
{
	if (enable)
 ...
 */
void rtl8822e_mp_config_rfpath(PADAPTER adapter)
{
	PHAL_DATA_TYPE hal;
	PMPT_CONTEXT mpt;
...
static void _sounding_config_su(PADAPTER adapter, struct beamformee_entry *bfee, enum _HW_CFG_SOUNDING_TYPE cfg_type)
{
	...
	case 0:
		new_ctrl &= ~(BIT_R_TXBF0_20M_8822E|BIT_R_TXBF0_40M_8822E|BIT_R_TXBF0_80M_8822E);
		break;
	case 1:
		new_ctrl &= ~(BIT_R_TXBF1_20M_8822E|BIT_R_TXBF1_40M_8822E|BIT_R_TXBF1_80M_8822E);
		break;
	}
	...
	case HW_CFG_SOUNDING_TYPE_SOUNDDOWN:
		...
		case CHANNEL_WIDTH_80:
			if (0 == bfee->su_reg_index)
				new_ctrl |= BIT_R_TXBF0_80M_8822E;
			else if (1 == bfee->su_reg_index)
				new_ctrl |= BIT_R_TXBF1_80M_8822E;
			fallthrough;
		case CHANNEL_WIDTH_40:
			if (0 == bfee->su_reg_index)
				new_ctrl |= BIT_R_TXBF0_40M_8822E;
			else if (1 == bfee->su_reg_index)
				new_ctrl |= BIT_R_TXBF1_40M_8822E;
			fallthrough;
		case CHANNEL_WIDTH_20:
			if (0 == bfee->su_reg_index)
				new_ctrl |= BIT_R_TXBF0_20M_8822E;
			else if (1 == bfee->su_reg_index)
				new_ctrl |= BIT_R_TXBF1_20M_8822E;
			break;
		}
		break;
	...
	if (new_ctrl != txbf_ctrl)
		rtw_write32(adapter, REG_TXBF_CTRL_8822E, new_ctrl);
}
...
static void _sounding_config_mu(PADAPTER adapter, struct beamformee_entry *bfee, enum _HW_CFG_SOUNDING_TYPE cfg_type)
{
	...
	case HW_CFG_SOUNDING_TYPE_LEAVE:
		...
		if (TEST_FLAG(bfee->cap, BEAMFORMEE_CAP_VHT_MU)) {
			...
			mu_tx_ctl = BIT_SET_R_MU_TAB_SEL_8822E(mu_tx_ctl, bfee->mu_reg_index);
			rtw_write32(adapter, REG_MU_TX_CTL_8822E, mu_tx_ctl);
			/* Set MU STA gid valid table */
			rtw_write32(adapter, REG_MU_STA_GID_VLD_8822E, 0);
		} else {
	...
	case HW_CFG_SOUNDING_TYPE_SOUNDDOWN:
		...
		for (i = 0; i < 6; i++) {
			...
			rtw_write32(adapter, REG_MU_TX_CTL_8822E, mu_tx_ctl);
			/* Set MU STA gid valid table */
			rtw_write32(adapter, REG_MU_STA_GID_VLD_8822E, gid_valid[i]);
		}
...
static void _config_beamformer_su(PADAPTER adapter, struct beamformer_entry *bfer)
{
	...
	if (bfer->su_reg_index == 0) {
		...
		addr_bfer_info = REG_ASSOCIATED_BFMER1_INFO_8822E;
		addr_csi_rpt = REG_TX_CSI_RPT_PARAM_BW20_8822E + 2;
	}
...
static void _config_beamformer_mu(PADAPTER adapter, struct beamformer_entry *bfer)
{
	...
	val8 = rtw_read8(adapter, REG_RXFLTMAP0_8822E+1);
	val8 |= (BIT_MGTFLT14EN_8822E >> 8);
	rtw_write8(adapter, REG_RXFLTMAP0_8822E+1, val8);

	/* Set 0x6A2[5:4] = 1 to NDPA and BF report poll */
	val8 = rtw_read8(adapter, REG_RXFLTMAP1_8822E);
	val8 |= BIT_CTRLFLT4EN_8822E | BIT_CTRLFLT5EN_8822E;
	rtw_write8(adapter, REG_RXFLTMAP1_8822E, val8);
...
static void _config_beamformee_su(PADAPTER adapter, struct beamformee_entry *bfee)
{
	...
	if (idx == 0) {
		...
		val32 = BIT_SET_R_TXBF1_AID_8822E(val32, p_aid);
		val32 &= ~(BIT_R_TXBF1_20M_8822E | BIT_R_TXBF1_40M_8822E | BIT_R_TXBF1_80M_8822E);
	}
	...
	if (idx == 0) {
		...
		val32 = BIT_SET_AID1_8822E(val32, p_aid);
		val32 |= BIT_TXUSER_ID1_8822E;
...
static void _config_beamformee_mu(PADAPTER adapter, struct beamformee_entry *bfee)
{
	...
	u32 mu_reg[6] = {REG_WMAC_ASSOCIATED_MU_BFMEE2_8822E,
			 REG_WMAC_ASSOCIATED_MU_BFMEE3_8822E,
			 REG_WMAC_ASSOCIATED_MU_BFMEE4_8822E,
			 REG_WMAC_ASSOCIATED_MU_BFMEE5_8822E,
			 REG_WMAC_ASSOCIATED_MU_BFMEE6_8822E,
			 REG_WMAC_ASSOCIATED_MU_BFMEE7_8822E};
	/* Misc */
	u8 i, val8;
	u16 val16;
	u32 val32;
	...
	val32 = BIT_SET_R_MU_TAB_SEL_8822E(val32, idx);
	rtw_write32(adapter, REG_MU_TX_CTL_8822E, val32);

	/* Reset gid_valid table */
	rtw_write32(adapter, REG_MU_STA_GID_VLD_8822E, 0);
	rtw_write32(adapter, REG_MU_STA_USER_POS_INFO_8822E , user_position_l);
	rtw_write32(adapter, REG_MU_STA_USER_POS_INFO_8822E+4 , user_position_h);

	/* set validity of MU STAs */
	val32 = BIT_SET_R_MU_TABLE_VALID_8822E(val32, info->beamformee_mu_reg_maping);
	rtw_write32(adapter, REG_MU_TX_CTL_8822E, val32);
	...
	val8 = rtw_read8(adapter, REG_TXBF_CTRL_8822E+3);
	val8 |= 0xD0; /* Set bit 28, 30, 31 to 3b'111 */
	rtw_write8(adapter, REG_TXBF_CTRL_8822E+3, val8);

	/* Set NDPA rate*/
	val8 = phydm_get_ndpa_rate(GET_PDM_ODM(adapter));
	rtw_write8(adapter, REG_NDPA_RATE_8822E, val8);

	val8 = rtw_read8(adapter, REG_NDPA_OPT_CTRL_8822E);
	val8 = BIT_SET_R_NDPA_BW_8822E(val8, 0); /* Clear bit 0, 1 */
	rtw_write8(adapter, REG_NDPA_OPT_CTRL_8822E, val8);

	val32 = rtw_read32(adapter, REG_SND_PTCL_CTRL_8822E);
	val32 = (val32 & 0xFF0000FF) | 0x020200; /* Set [23:8] to 0x0202 */
	rtw_write32(adapter, REG_SND_PTCL_CTRL_8822E, val32);
	...
	val8 = rtw_read8(adapter, REG_SND_PTCL_CTRL_8822E+1);
	val8 |= (BIT_VHTNDP_RPTPOLL_CSI_STR_OFFSET_SEL_8822E >> 8);
	rtw_write8(adapter, REG_SND_PTCL_CTRL_8822E+1, val8);
...
static void _reset_beamformee_su(PADAPTER adapter, struct beamformee_entry *bfee)
{
	...
	if (idx == 0) {
		...
		txbf_ctrl = BIT_SET_R_TXBF1_AID_8822E(txbf_ctrl, 0);
		txbf_ctrl &= ~(BIT_R_TXBF1_20M_8822E | BIT_R_TXBF1_40M_8822E | BIT_R_TXBF1_80M_8822E);

		bfmee_sel = BIT_SET_AID1_8822E(bfmee_sel, 0);
		bfmee_sel &= ~BIT_TXUSER_ID1_8822E;
	}
...
void rtl8822e_phy_bf_reset_all(PADAPTER adapter)
{
	struct beamforming_info *info;
	u8 i, val8;
	u32 val32;
...
void rtl8822e_phy_bf_init(PADAPTER adapter)
{
	u8 v8;
	u32 v32;
...
void rtl8822e_phy_bf_enter(PADAPTER adapter, struct sta_info *sta)
{
	struct beamforming_info *info;
...
void rtl8822e_phy_bf_leave(PADAPTER adapter, u8 *addr)
{
	struct beamforming_info *info;
	struct beamformer_entry *bfer;
	struct beamformee_entry *bfee;
...
void rtl8822e_phy_bf_set_gid_table(PADAPTER adapter,
...
void rtl8822e_phy_bf_sounding_status(PADAPTER adapter, u8 status)
{
	struct beamforming_info	*info;
	...
	if (sounding->state == SOUNDING_STATE_SU_SOUNDDOWN) {
		...
		val16 = rtw_read16(adapter, REG_TXBF_CTRL_8822E);
		val16 |= BIT_DIS_NDP_BFEN_8822E;
		rtw_write16(adapter, REG_TXBF_CTRL_8822E, val16);
	} else if (sounding->state == SOUNDING_STATE_MU_SOUNDDOWN) {
		/* MU sounding done */
		RTW_INFO("%s: Config MU sound down HW settings\n", __FUNCTION__);

		val32 = rtw_read32(adapter, REG_WMAC_ASSOCIATED_MU_BFMEE2_8822E);
		is_sounding_success[0] = (val32 & BIT_STATUS_BFEE2_8822E) ? 1:0;
		is_sounding_success[1] = ((val32 >> 16) & BIT_STATUS_BFEE3_8822E) ? 1:0;
		val32 = rtw_read32(adapter, REG_WMAC_ASSOCIATED_MU_BFMEE4_8822E);
		is_sounding_success[2] = (val32 & BIT_STATUS_BFEE4_8822E) ? 1:0;
		is_sounding_success[3] = ((val32 >> 16) & BIT_BIT_STATUS_BFEE5_8822E) ? 1:0;
		val32 = rtw_read32(adapter, REG_WMAC_ASSOCIATED_MU_BFMEE6_8822E);
		is_sounding_success[4] = (val32 & BIT_STATUS_BFEE6_8822E) ? 1:0;
		is_sounding_success[5] = ((val32 >> 16) & BIT_STATUS_BFEE7_8822E) ? 1:0;
